...
public class commonMethods {
    ...
    public void checkIfBookingIsAllowed(String date) {
        ...
        WebDriverWaitUtils.waitUntilElementIsVisible(homePage
                .getFromUlTextField().getLocator());
        String locator = homePage.getFromLiTextField().getLocator();
        locator = locator.split("css=")[1];
        java.util.List<WebElement> elements = Grid.driver().findElements(
                By.cssSelector(locator));
        Boolean optionFound = false;
        for (WebElement element : elements) {
            if (element.findElement(By.tagName("a")).getText()
                    .contains("NIZAMABAD - NZB")) {
                optionFound = true;
        ...
        for (WebElement element : elements) {
            if (element.findElement(By.tagName("a")).getText()
                    .contains("BNG KEMPEGOWDA BS TERMINAL 1 - BENGALURE")) {
                optionFound = true;
                element.findElement(By.tagName("a")).click();
                break;
            }
        ...
        WebDriverWaitUtils.waitUntilElementIsVisible(selectBusPage
                .getBackButton().getLocator());

        Boolean found = SeleniumUtils.isElementVisible(selectBusPage
                .getOddRowTextField().getLocator());
        ...
        if (found) {
            ...
            oneSixtyBy2LoginPage.clickSendFreeSMSButton(oneSixtyBy2LoginPage
                    .getEnterMobileNumberOrNameTextField().getLocator());
            oneSixtyBy2LoginPage
                    .setEnterMobileNumberOrNameTextFieldValue("8985488161");
